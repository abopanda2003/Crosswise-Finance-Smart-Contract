...
import "../../session/interfaces/ISessionConstants.sol";
...
library ChainLink {
    ...
    function initializeBnbMainNetCLFeeds(mapping(address => CLFeed) storage chainlinkFeeds) public {
        // AAPL / USD 
    ...
    function initializeBnbTestNetCLFeeds(mapping(address => CLFeed) storage chainlinkFeeds) public {/* BNB test net */
        // AAVE / USD 
    ...
    int256 constant posExponent = int256(10 ** (23 + 2 * FeeMagnifierPower + 8));
    int256 constant negExponent = int256(10 ** (23 - 2 * FeeMagnifierPower - 8));
    function getChainLinkPrice1e23Range(mapping(address => CLFeed) storage chainlinkFeeds, address token0, address token1, int256 zValue) public view 
    returns (int256 min1e1e23, int256 max1e23) {
        CLFeed memory priceFeed0 = chainlinkFeeds[token0];
        require( priceFeed0.proxy != address(0), "CrossRouter: ChianLink not fnound");
        ...
        require( priceFeed1.proxy != address(0), "CrossRouter: ChianLink not fnound");
        ...
        // max = price + zValue / 10 ** M * sigma = price * E, as zValue is the z tale of Gaussian distribution.
        // min = price / E, as price has logarithmic characteristics.
        // E = 1 + zValue * priceFeed.deviation * 1e (-M-4);
        // min1eD = min0 * 1eD / max1 = R0 * 1e (D+2M+8) / R1 / G, D > 5
        // max1eD = max0 * 1eD / min1 = R0 * 1e (D-2M-8) / R1 * G, D > 2M + 8
        // G = 10 ** (2M+8) * (1 + zValue * priceFeed0.deviation * 10**(-M-4) ) (1 + zValue * priceFeed1.deviation * 10**(-M-4))
        // Let D be 23.
        // R0, R1: price value returned by ChainLink.
        // 10**M: FeeMagnifier.

        int256 Mike = SquareM4 + ( (d0 + d1) * LinearM4 + zValue * (d0 * d1) ) * zValue;
        int256 min1e23 = price0 * posExponent / Mike / price1;
        int256 max1e23 = price0 * negExponent * Mike / price1;
    }
}
